...
t_vec_days = (t_vec - t0) / 86400

_, _, l_vec = rnp_rpf_coeff.compute_times_distances(tau_0, tau_f, t0, n_points, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame, abcorr_flag)


t_int = rnp_rpf_coeff.compute_time(tau_0, tau_f, t0, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame, abcorr_flag)
t_int_days = (t_int - t0) / 86400
...
tau = 53.678
# tau = 25
t_int_tau = rnp_rpf_coeff.compute_time(tau_0, tau, t0, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame, abcorr_flag)
t_int_tau_s = t_int_tau - t0
t_int_tau_days = (t_int_tau - t0) / 86400

t_int_tau_interp = np.interp(tau, tau_vec, t_vec - t0)
t_int_tau_interp_days = np.interp(tau, tau_vec, (t_vec - t0)/86400)
# t_int_tau_interp_days = (t_int_tau_interp - t0) / 86400

f_interp = interpolate.interp1d(tau_vec, t_vec - t0, kind = 'cubic')
...
tau_comp = compute_tau(t, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame, abcorr_flag)
...
rnbprpf_dict["param"]["t_vec"] = t_vec - t0
