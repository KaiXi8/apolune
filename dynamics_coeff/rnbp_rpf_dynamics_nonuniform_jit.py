...
from utils.libs.cspice_wrapper import lib
import utils.libs.cspice_wrapper as cspice_wrapper
...
# 
# @jit('void(float64, int64[::1], int64, int8[::1], float64[:,::1], float64[::1])', nopython=True, nogil=True, fastmath=True)
# def get_body_positions(t, naif_id_bodies, observer_id, reference_frame, pos_bodies, lt_ob):
...
# @jit('void(float64, int64[::1], int64, int8[::1], float64[:,::1], float64[::1])', nopython=True, nogil=True, fastmath=True)
# def get_body_states(t, naif_id_bodies, observer_id, reference_frame, state_bodies, lt_ob):
#     num_bodies = len(naif_id_bodies)
# 
#     # retrieve states of celestial bodies from SPICE
#     for i in prange(num_bodies):
...
@jit('Tuple((float64[::1], float64[::1], float64, float64, float64[::1], float64[::1], float64, float64, float64, float64[:, ::1]))'
     '(int64, int64, float64[:, ::1], float64[:, ::1], float64[::1])', nopython=True, nogil=True, fastmath=True)
...
@jit('Tuple((float64[::1], float64[::1], float64, float64, float64, float64[:, ::1], float64[:, ::1], float64[:, ::1]))'
     '(int64, int64, float64[:, ::1], float64[:, ::1], float64[::1])', nopython=True, nogil=True, fastmath=True)
...
@jit('float64[:,::1](float64[:,::1], float64[::1], float64[:,::1], float64)', nopython=True, nogil=True, fastmath=True)
...
@jit('float64[::1](float64[::1], float64[:,::1], float64[::1], float64[:,::1], float64, float64[::1])', nopython=True, nogil=True, fastmath=True)
...
@jit('(float64[::1])'
     '(float64, int64, int64, float64[::1], int64[::1], int64, int8[::1], float64, float64[::1], float64[::1])', nopython=True, nogil=True, fastmath=True)
def compute_coeffs(tau, id_primary, id_secondary, mu_bodies_dim, naif_id_bodies, observer_id, reference_frame, epoch_t0, tau_vec, t_vec):    
    ...
    b6 = 2*distance / (tau_prime*h) * np.dot(acc_p_s, e3)
    b7 = -distance_double_prime / (tau_prime_squared*distance) + h**2 / (tau_prime_squared*distance**4)
    b8 = -1 / (tau_prime_squared*distance) * np.dot(acc_p_s, e3)
    b9 = h1d / (tau_prime_squared*distance**2)
    b10 = -distance_double_prime / (tau_prime_squared*distance) + h**2 / (tau_prime_squared*distance**4) + distance**2 / (tau_prime_squared * h**2) * (np.dot(acc_p_s, e3))**2
    b11 = (3*h*distance_prime - 2*distance*h1d ) * np.dot(acc_p_s, e3) / (tau_prime_squared * h**2) + distance * np.dot(jerk_p_s, e3) / (tau_prime_squared * h)
    b12 = -distance_double_prime / (tau_prime_squared*distance) +  distance**2 / (tau_prime_squared * h**2) * (np.dot(acc_p_s, e3))**2
    b13 = (mu_p + mu_s) / (distance**3 * tau_prime_squared)
    
    return np.array([b1, b2, b3, b4, b5, b6, b7, b8, b9, b10, b11, b12, b13])
