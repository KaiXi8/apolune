...
def get_body_distance(t, naif_id_p, naif_id_s, ref_frame):
...
def get_body_distance_ndim(time_vec, naif_id_p, naif_id_s, ref_frame):
    num_time = len(time_vec)
    l = np.zeros(num_time)
    
    for i in range(num_time):
        l[i] = get_body_distance(time_vec[i], naif_id_p, naif_id_s, ref_frame)
...
#         l = get_body_distance_ndim(t, naif_id_p, naif_id_s, ref_frame)
#     else:
#         l = get_body_distance_1dim(t, naif_id_p, naif_id_s, ref_frame)
...
def dt_dtau(tau, t, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame):
    l = get_body_distance(t, naif_id_p, naif_id_s, ref_frame)
...
#     l_vec = get_body_distance(sol_y + et0, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame)
    l_vec = get_body_distance_ndim(sol_y + et0, naif_id_p, naif_id_s, ref_frame)
...
#     l_vec = get_body_distance(t_vec + et0, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame)
    l_vec = get_body_distance_ndim(t_vec + et0, naif_id_p, naif_id_s, ref_frame)
    
    return sol_t, sol_y, l_vec



@jit('Tuple((float64, float64, float64, float64, float64))(float64, float64[::1], float64[::1], float64, float64, float64)', nopython=True, nogil=True, fastmath=True)
def get_scaling_param(t, t_vec, l_vec, mu_p, mu_s, MU):
    LU = np.interp(t, t_vec, l_vec) # [km]
    TU = np.sqrt(LU**3 / (mu_p + mu_s)) # [s]
    VU = LU / TU # [km/s]
    ACU = VU / TU # Acceleration unit [km/s^2]
    FU = MU * ACU / 1e-3
    
    return LU, TU, VU, ACU, FU


@jit('Tuple((float64[::1], float64[::1], float64[::1], float64[::1], float64[::1]))(float64[::1], float64[::1], float64[::1], float64, float64, float64)', nopython=True, nogil=True, fastmath=True)
def get_scaling_param_ndim(t, t_vec, l_vec, mu_p, mu_s, MU):
    LU = np.interp(t, t_vec, l_vec) # [km]
    TU = np.sqrt(LU**3 / (mu_p + mu_s)) # [s]
    VU = LU / TU # [km/s]
    ACU = VU / TU # Acceleration unit [km/s^2]
...
def get_scaling_FU(t, t_vec, l_vec, mu_p, mu_s, MU):
    _, _, _, _, FU = get_scaling_param(t, t_vec, l_vec, mu_p, mu_s, MU)
    
    return FU


def get_scaling_FU_ndim(t, t_vec, l_vec, mu_p, mu_s, MU):
    _, _, _, _, FU = get_scaling_param_ndim(t, t_vec, l_vec, mu_p, mu_s, MU)
...
# 
# 
# def get_body_distance_ndim(time_vec, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame):
#     num_time = len(time_vec)
#     l = np.zeros(num_time)
#     
#     for i in range(num_time):
#         l[i] = get_body_distance(time_vec[i], mu_p, mu_s, naif_id_p, naif_id_s, ref_frame)
#         
#     return l
# 
# 
...
# # def get_body_distance(t, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame):
# #     if len(np.atleast_1d(t)) > 1:
# #         l = get_body_distance_ndim(t, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame)
# #     else:
# #         l = get_body_distance_1dim(t, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame)
# 
# # def get_body_distance(t, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame):
# #     pos_p_s, _ = spice.spkgps(naif_id_s, t, ref_frame, naif_id_p)
# #     
...
# 
# #     l_vec = get_body_distance(sol_y + et0, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame)
#     l_vec = get_body_distance_ndim(sol_y + et0, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame)
#     
#     return sol_t, sol_y, l_vec
#     
# 
...
#     sol_t, sol_y = compute_tau(t_vec, tau0, et0, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame, ode_rtol = ode_rtol, ode_atol = ode_atol)
# #     l_vec = get_body_distance(t_vec + et0, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame)
#     l_vec = get_body_distance_ndim(t_vec + et0, mu_p, mu_s, naif_id_p, naif_id_s, ref_frame)
#     
#     return sol_t, sol_y, l_vec