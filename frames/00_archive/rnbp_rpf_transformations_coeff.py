...
def frame_rnbprpf_transform(id_primary, id_secondary, pos_bodies, vel_bodies, num_bodies, mu_bodies):
    ...
    for idx, i in enumerate((id_primary, id_secondary)):
        for j in range(num_bodies):
            ...
            acc = (mu_bodies[j] / norm_R**3) * R_vec # [km^3/s^2]
...
def inertialToSynodic(time, state_inertial, rnbprpf_dict):
    ...
    mu_p = mu_bodies[id_primary] # [km^3 / s^2]
    mu_s = mu_bodies[id_secondary] # [km^3 / s^2]
    ...
    for i in range(num_time):
        ...
        pos_bodies = pos_bodies_all[i, :, :] # [adim]
        vel_bodies = vel_bodies_all[i, :, :] # [adim]
        ...
        tau_prime = np.sqrt((mu_p + mu_s) / (k**3)) # [1/s]

        # transform inertial to synodic coordinates (eq. 3.1a)
        # rho_synodic[i, :] = C.T @ (pos_inertial[i] - b) / k # [adim]
#         eta_synodic[i, :] = C.T @ (vel_inertial[i] - b1d - (k * C1d + k1d * C) @ rho_synodic[i, :]) / (k * tau_prime) # [adim]

        rho_synodic[i, :] = (pos_inertial[i] - b) @ C / k # [adim]
        eta_synodic[i, :] = (vel_inertial[i] - b1d - (k * C1d + k1d * C) @ rho_synodic[i, :]) @ C / (k * tau_prime) # [adim]
...
def synodicToInertial(tau, state_synodic, rnbprpf_dict):
    ...
    for i in range(num_time):
        ...
        tau_prime = np.sqrt((mu_p + mu_s) / (k**3))

        # transform synodic to inertial coordinates (eq. 3.1a)
        pos_inertial[i, :] = b + k * rho_synodic[i,:] @ C.T # [km]
        vel_inertial[i, :] = b1d + ( (k1d*C + k*C1d) @ rho_synodic[i,:] + k*C*tau_prime @ eta_synodic[i,:] ) # [km/s]
