...
import multiple_shooting.objective
...
import crtbp_input_example_3_diff_corr as diff_corr


# get user input data
auxdata = diff_corr.settings()
...
tspan = np.array([time[0], 2*time[-1]])
...
ode_sol = propagation.propagate(crtbp.dynamics, x0_sol, 0.0, p_sol, tspan, auxdata)
...
ax.plot(x_prop[0,:], x_prop[1,:], x_prop[2,:], label='transfer')
ax.set_xlabel("x")
ax.set_ylabel("y")
ax.set_zlabel("z")
plt.legend()
...
fig, ax = plt.subplots()
ax.plot(x_prop[0,:], x_prop[1,:], label='transfer')
ax.set_xlabel("x")
ax.set_ylabel("y")
