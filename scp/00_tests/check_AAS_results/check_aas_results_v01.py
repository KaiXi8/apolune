...
import scp.scp_solve as scp_solve
...
state_guess = np.load('state_sol.npy') 
control_guess = np.load('control_sol.npy') 
p_guess = np.empty(0)
...
if man_index[0] == 0:
    dx0 = state_sol[0] - np.concat((np.zeros(3), control_sol[0])) - x0
else:
    dx0 = state_sol[0] - x0
violation_x0 = np.linalg.norm(dx0, 1)
print("violation_x0: ", violation_x0)

dxf = state_sol[-1] - xf
violation_xf = np.linalg.norm(dxf, 1)
print("violation_xf: ", violation_xf)
  
dv_violations = np.maximum(0, np.linalg.norm(control_sol, axis=1) - dv_max)
print("dv_violations: ", dv_violations)

d_state = state_prop - state_sol
pos_error = np.linalg.norm(d_state[:, :3], axis=1)
vel_error = np.linalg.norm(d_state[:, 3:6], axis=1)
pos_error_max = np.max(pos_error)
vel_error_max = np.max(vel_error)

print("propagation error, adim: ", d_state[-1])
...
plt.figure()
ax = plt.axes(projection ='3d')
ax.plot(state_sol[:,0], state_sol[:,1], state_sol[:,2], label='SCP solution')
# ax.plot(state_guess[:,0], state_guess[:,1], state_guess[:,2], label='guess')
ax.plot(state_prop[:,0], state_prop[:,1], state_prop[:,2], label='propagated')
ax.set_xlabel("x")
ax.set_ylabel("y")
ax.set_zlabel("z")
plt.legend()
...
fig, ax = plt.subplots()
ax.plot(state_sol[:,0], state_sol[:,1], label='SCP solution')
# ax.plot(state_guess[:,0], state_guess[:,1], label='guess')
ax.plot(state_prop[:,0], state_prop[:,1], label='propagated')
ax.set_xlabel("x")
ax.set_ylabel("y")
