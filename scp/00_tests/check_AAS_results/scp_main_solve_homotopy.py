...
scp_data['feasibility_tol'] = 1e-2#1e-7
scp_data['optimality_tol'] = 1e-2#1e-4
scp_data['step_tol'] = 1e-3#1e-8
...
while converged_flag != 1:
    ...
    if solution_data['converged_flag'] == 1: # found an optimal solution -> increase homotopic parameter
        ...
        scp_data['feasibility_tol'] *= 1e-6 ** (1/10)
        scp_data['optimality_tol'] *= 1e-2 ** (1/10)
        scp_data['step_tol'] *= 1e-5 ** (1/10)
...
np.save("state_sol.npy", state_sol)
np.save("control_sol.npy", control_sol)
