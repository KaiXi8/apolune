...
import cvxpy as cvx
from dynamics_coeff.rnbp_rpf_utils import get_scaling_FU_ndim
...
def integrate_piecewise(dynamics_fun, tvec, x, u, p, auxdata):
...
#     if model == 1 or model == 2 or model == 3: # crtbp or bcrfbp
        ...
        Tmax_dim = auxdata['param']['Tmax_dim']
        tau_interp_vec = auxdata['tau_vec']
        l_interp_vec = auxdata['l_vec']
        mu_p = auxdata['mu_p']
        mu_s = auxdata['mu_s']
        mu_s = auxdata['mu_s']
        MU = auxdata['scaling']['MU']
        
        FU = get_scaling_FU_ndim(time, tau_interp_vec, l_interp_vec, mu_p, mu_s, MU)
...
#     Tmax = auxdata['param']['Tmax']
    ...
    total_con_violation = violation_dynamics + violation_x0 + violation_xf \
        + umag_violations_total + control_norm_violations_total + umag_bounds_violations_total
    
    con_violation_concat = np.concatenate((defects_matrix.flatten(), dx0, dxf, umag_violations, 
        control_norm_violations, umag_bounds_violations))
...
def calc_linear_cost(tvec, state, control, p, slack_dynamics, slack_ineq, auxdata):
    n_x = auxdata['problem']['n_x']
    n_u = auxdata['problem']['n_u']
    
    violation_dynamics = np.array(slack_dynamics)
    violation_dynamics_total = np.linalg.norm(violation_dynamics.flatten(), 1)
    ...
    umag_violations = np.maximum(0, slack_ineq)
    umag_violations_total = np.sum(umag_violations)
    
    umag_bounds_violations = np.minimum(0, u_mag)
    umag_bounds_violations_total = np.sum(umag_bounds_violations)
    
    control_norm_violations = np.maximum(0, np.linalg.norm(control[:, :3], axis=1) - u_mag)
    control_norm_violations_total = np.sum(control_norm_violations)
    
    total_con_violation = violation_dynamics_total + violation_x0 + violation_xf \
        + umag_violations_total + control_norm_violations_total + umag_bounds_violations_total
        
    con_violation_concat = np.concatenate((violation_dynamics.flatten(), dx0, dxf, umag_violations, 
        control_norm_violations, umag_bounds_violations))
    max_con_violation = np.linalg.norm(con_violation_concat, np.inf)
